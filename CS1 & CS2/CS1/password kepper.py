...
Date: 5/15/2024
Description: Stores your usernames and passwords for websites in a trusted program, allowing you to activily change them, view them, and view them in an excel sheet!
Bugs: None
...
def main(): #Creates a function
    ...
    while True:
        ...
        if mode =="6":
            ...
            for i in range(len(websites)): #A for loop which finds all the indexes in websites 
                print_account(websites, usernames, passwords, i)
        elif mode =="3":
            web = input("Enter a website: ")

            if web in websites:
                i = websites.index(web)# Finds the individual index for the inputed website 
                print_account(websites, usernames, passwords, i)
        elif mode =="4":
            ...
            columns = {} #Starts the columns list 
            columns['websites'] = websites #Names each column 
            columns['usernames'] = usernames #Names each column 
            columns['passwords'] = passwords #Names each column 

            rows = zip(columns['websites'],columns['usernames'], columns['passwords']) #Inserts the indivudual columns 

            with open('my_file.csv','w',newline='') as f: #Opens a sheet challed my_file 
                writer = csv.writer(f) 
